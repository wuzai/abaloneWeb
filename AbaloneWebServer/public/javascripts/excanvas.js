...
document.createElement("canvas").getContext || function () {
  ...
  function w(e) {
    ...
    if (e.substring(0, 3) == "rgb") {
      ...
      t = "#";
      for (var o = 0; o < 3; o++)t += d[Number(s[o])];
      s.length == 4 && e.substr(3, 1) == "a" && (n = s[3])
    } else t = e;
  ...
  function S(e) {
    this.m_ = g(), this.mStack_ = [], this.aStack_ = [], this.currentPath_ = [], this.strokeStyle = "#000", this.fillStyle = "#000", this.lineWidth = 1, this.lineJoin = "miter", this.lineCap = "butt", this.miterLimit = o * 1, this.globalAlpha = 1, this.canvas = e;
  ...
  function N(e) {
    for (var t = 0; t < 3; t++)for (var n = 0; n < 2; n++)if (!isFinite(e[t][n]) || isNaN(e[t][n]))return!1;
    return!0
  }
  ...
  var e = Math, t = e.round, n = e.sin, r = e.cos, i = e.abs, s = e.sqrt, o = 10, u = o / 2, f = Array.prototype.slice, c = {init:function (e) {
    if (/MSIE/.test(navigator.userAgent) && !window.opera) {
      var t = e || document;
      t.createElement("canvas"), t.readyState !== "complete" ? t.attachEvent("onreadystatechange", l(this.init_, this, t)) : this.init_(t)
    }
  }, init_                                                                                                                       :function (e) {
    ...
    for (var r = 0; r < n.length; r++)this.initElement(n[r])
  }, initElement                                                                                                                 :function (e) {
    if (!e.getContext) {
      e.getContext = a, e.innerHTML = "", e.attachEvent("onpropertychange", h), e.attachEvent("onresize", p);
      var t = e.attributes;
  ...
  c.init();
  var d = [];
  for (var v = 0; v < 16; v++)for (var m = 0; m < 16; m++)d[v * 16 + m] = v.toString(16) + m.toString(16);
  var x = S.prototype;
  x.clearRect = function () {
    ...
    var i = this.getCoords_(e, t), s = this.getCoords_(n, r), o = {x:this.currentX_ + 2 / 3 * (i.x - this.currentX_), y:this.currentY_ + 2 / 3 * (i.y - this.currentY_)}, u = {x:o.x + (s.x - this.currentX_) / 3, y:o.y + (s.y - this.currentY_) / 3};
    T(this, o, u, s)
    ...
    var g = this.getCoords_(i, s), y = c / 2, b = h / 2, w = [], E = 10, S = 10;
    ...
    if (this.m_[0][0] != 1 || this.m_[0][1]) {
      ...
      var T = g, N = this.getCoords_(i + u, s), C = this.getCoords_(i, s + a), k = this.getCoords_(i + u, s + a);
    ...
    var r = [], i = !1, s = w(n ? this.fillStyle : this.strokeStyle), u = s.color, a = s.alpha * this.globalAlpha, f = 10, l = 10;
    ...
    var c = !1, h = {x:null, y:null}, p = {x:null, y:null};
    for (var d = 0; d < this.currentPath_.length; d++) {
      var v = this.currentPath_[d], m;
      switch (v.type) {
        case"moveTo":
          m = v, r.push(" m ", t(v.x), ",", t(v.y));
          break;
        case"lineTo":
      ...
      if (v) {
        if (h.x == null || v.x < h.x)h.x = v.x;
        if (p.x == null || v.x > p.x)p.x = v.x;
        if (h.y == null || v.y < h.y)h.y = v.y;
        if (p.y == null || v.y > p.y)p.y = v.y
      }
    }
    r.push(' ">');
    if (!n) {
      var g = this.lineScale_ * this.lineWidth;
      g < 1 && (a *= g), r.push("<g_vml_:stroke", ' opacity="', a, '"', ' joinstyle="', this.lineJoin, '"', ' miterlimit="', this.miterLimit, '"', ' endcap="', E(this.lineCap), '"', ' weight="', g, 'px"', ' color="', u, '" />')
    } else if (typeof this.fillStyle == "object") {
      var y = this.fillStyle, b = 0, S = {x:0, y:0}, x = 0, T = 1;
      if (y.type_ == "gradient") {
        var N = y.x0_ / this.arcScaleX_, C = y.y0_ / this.arcScaleY_, k = y.x1_ / this.arcScaleX_, L = y.y1_ / this.arcScaleY_, A = this.getCoords_(N, C), O = this.getCoords_(k, L), M = O.x - A.x, _ = O.y - A.y;
        b = Math.atan2(M, _) * 180 / Math.PI, b < 0 && (b += 360), b < 1e-6 && (b = 0)
      } else {
        var A = this.getCoords_(y.x0_, y.y0_), D = p.x - h.x, P = p.y - h.y;
        S = {x:(A.x - h.x) / D, y:(A.y - h.y) / P}, D /= this.arcScaleX_ * o, P /= this.arcScaleY_ * o;
        var H = e.max(D, P);
        x = 2 * y.r0_ / H, T = 2 * y.r1_ / H - x
      }
      var B = y.colors_;
      B.sort(function (e, t) {
        return e.offset - t.offset
      });
      var j = B.length, F = B[0].color, I = B[j - 1].color, q = B[0].alpha * this.globalAlpha, R = B[j - 1].alpha * this.globalAlpha, U = [];
      for (var d = 0; d < j; d++) {
        var z = B[d];
    ...
    } else r.push('<g_vml_:fill color="', u, '" opacity="', a, '" />');
    r.push("</g_vml_:shape>"), this.element_.insertAdjacentHTML("beforeEnd", r.join(""))
  }, x.fill = function () {
    this.stroke(!0)
    ...
    var e = {};
    b(this, e), this.aStack_.push(e), this.mStack_.push(this.m_), this.m_ = y(g(), this.m_)
  }, x.restore = function () {
    b(this.aStack_.pop(), this), this.m_ = this.mStack_.pop()
    ...
    t = w(t), this.colors_.push({offset:e, color:t.color, alpha:t.alpha})
  }, G_vmlCanvasManager = c, CanvasRenderingContext2D = S, CanvasGradient = k, CanvasPattern = L
}();